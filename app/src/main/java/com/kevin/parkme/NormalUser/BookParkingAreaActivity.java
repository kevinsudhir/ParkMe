package com.kevin.parkme.NormalUser;
...
import com.kevin.parkme.classes.NumberPlate;
                            ...
                            new DialogInterface.OnClickListener() {
                                ...
                                public void onClick(DialogInterface dialog, int which) {
                                    if(parkingArea.availableSlots>0) {
                                        ...
                                        intent=new Intent(BookParkingAreaActivity.this, CardPayment.class);
                                        Bundle b = new Bundle();
                                        b.putInt("amount", bookingSlot.amount); //Your amount
                                        intent.putExtras(b); //Put your amount to your next Intent
        ...
        db.getReference().child("ParkingAreas").child(bookingSlot.placeID)
                .addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        ...
                        db.getReference().child("OwnerAccountInfo").child(parkingArea.userID).addValueEventListener(new ValueEventListener() {
                            @Override
                            public void onDataChange(@NonNull DataSnapshot snapshot) {
                                ownerAccountInfo=snapshot.getValue(OwnerAccountInfo.class);
                            }
                            @Override
                            public void onCancelled(@NonNull DatabaseError error) {}
                        });
        ...
        db.getReference("BookedSlots").child(key).setValue(bookingSlot).addOnCompleteListener(new OnCompleteListener<Void>() {
            ...
            public void onComplete(@NonNull Task<Void> task) {
                if(task.isSuccessful()){
                    ...
                    File file = new File(Environment.getExternalStorageDirectory()+"/Download", "invoice.pdf");
                    if(file.exists())
                    {
                        file.delete();
                    }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == code) {
            if (resultCode == RESULT_OK) {
                if(data.getStringExtra("n1").equals("true"))
                {
